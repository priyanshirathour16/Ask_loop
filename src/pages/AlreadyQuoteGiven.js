      ...
      try {
        ...
        quoteData.forEach(async (quote, index) => {
          try {
            ...
            console.error(
              `Error fetching details for ref_id ${quote.ref_id}`,
              err,
            );
          }
        });
          ...
          >
            ...
            >
              ...
              {quoteDetails.map((quote, index) => {
                ...
                const serviceIds = quote?.service_name
                  ? quote.service_name.split(",").map((id) => id.trim())
                  : [];
                ...
                return (
                  <div
                    key={index}
                    className="mb-8 text-gray-800 overflow-y-scroll pr-3 space-y-8 py-6"
                  >
                    <div className="flex items-center">
                      {quote.website_name && (
                        <p className="flex items-center text-blue-500 bg-gray-100 px-1 rounded border border-gray-200 py-0.5">
                          {" "}
                          <Globe
                            size={15}
                      ...
                      {quote.service_name && (
                        <p className="flex items-center text-blue-500 ml-1 bg-gray-100 px-1 rounded border border-gray-200 py-0.5">
                          {" "}
                          <FileText
                            size={15}
                            className="text-blue-600 mr-1"
                              ...
                              <tr key={i} className="hover:bg-gray-50">
                                ...
                                <td className="border px-3 py-2">
                                  {quote.tasks?.map((task, index) => {
                                    const prices =
                                      task.quote_price?.split(",") || [];
                                    return (
                                      <div key={index}>{prices[i] ?? "-"}</div>
                                    );
                                  })}
                                </td>
                                ...
                                <td className="border px-3 py-2">
                                  {quote.tasks?.map((task, index) => {
                                    const finals =
                                      task.final_price?.split(",") || [];
                                    return (
                                      <div key={index}>{finals[i] ?? "-"}</div>
                                    );
                                  })}
                                </td>
                              </tr>
