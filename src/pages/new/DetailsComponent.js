import { CheckCircle, CheckCircle2, ClockAlert, Expand, Link, Minimize2, Paperclip, Pen, TvMinimalPlay, X, XCircle } from "lucide-react";
...
const DetailsComponent = ({
    ...
    const renderPlanCard = (label, info, isRecommended) => {
        if (!info) return null;
        const hasMilestones = Array.isArray(info.milestoneData) && info.milestoneData.length > 0;
        return (
            <div className={`mb-2 p-2 relative border rounded ${label == "Basic" ? "bg-blue-50" : label == "Standard" ? "bg-gray-50" : "bg-yellow-50"}`}>
                <div className="flex items-center justify-between mb-1">
                    <strong className="text-md">{label}</strong>
                    {isRecommended ? <span className="badge badge-success f-10 absolute -top-2 right-0">Recommended</span> : null}
                </div>
                <div className="f-11 text-gray-700 space-y-2">
                    {info.price != null ? (
                        <p><strong>Total Price:</strong> {info.currency} {info.price} </p>
                    ) : null}
                    {info.milestones != null ? (
                        <p><strong>No. of Milestones:</strong> {info.milestones}</p>
                    ) : null}
                    {info.noOfWords != null && info.noOfWords !== "" ? (
                        <p><strong>No. of Words:</strong> {info.noOfWords}</p>
                    ) : null}
                    {hasMilestones ? (
                                ...
                                {info.milestoneData.map((milestone, idx) => (
                                    <div key={idx} className="p-2 bg-white rounded border">
                                        <div className="flex items-center justify-between">
                                            <span className="font-semibold">Name : {milestone.name}</span>
                                            <span className="ml-2">Price : {milestone.price != null ? <span className="ml-2">{info.currency} {milestone.price}</span> : null}</span>

                                        </div>
                                        {Array.isArray(milestone.parametersData) && milestone.parametersData.length > 0 ? (
                                            <div className="mt-1 f-11">
                                                <div className="overflow-x-auto">
                                                    <table className="min-w-full table-fixed border-collapse">
                                                        <thead>
                                                            <tr className="bg-gray-100">
                                                                <th className="border px-2 py-1 text-left w-1/3">Parameter</th>
                                                                <th className="border px-2 py-1 text-left w-1/3">No. of Words</th>
                                                                <th className="border px-2 py-1 text-left w-1/3">Time Frame</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            {milestone.parametersData.map((param, pIdx) => (
                                                                <tr key={pIdx}>
                                                                    <td className="border px-2 py-1 align-top break-words whitespace-normal">{param.parameters || '-'}</td>
                                                                    <td className="border px-2 py-1 align-top break-words whitespace-normal">{param.no_of_words || '-'}</td>
                                                                    <td className="border px-2 py-1 align-top break-words whitespace-normal">{param.time_frame || '-'}</td>
                                                                </tr>
                                                            ))}
                                                        </tbody>
                                                    </table>
                    ...
                    {info.remarks ? (
                        <p className="mt-1"><strong> Remarks for {label}:</strong> {info.remarks}</p>
                    ) : null}
                    {info.tandc ? (
                        <div className="mt-1">
                ...
                {canEdit && (
                    <div
                        ...
                        onClick={() => {
                            const rawInfo = label === "Basic" ? quote.basic_info : label === "Standard" ? quote.standard_info : quote.advanced_info;
                            console.log({
    ...
    const handleSave = async (payload) => {
        try {
            const response = await fetch('https://loopback-skci.onrender.com/api/scope/updatePlanInfoById', {
                method: 'POST',
    ...
    const handleApprove = async () => {
        try {
            const response = await fetch('https://loopback-skci.onrender.com/api/scope/approveRequestQuoteApiActionNewVersion', {
                method: 'POST',
                body: JSON.stringify({
    ...
    const handleOpsApprove = async () => {
        try {
            const response = await fetch('https://loopback-skci.onrender.com/api/scope/approveOpsRequestQuoteApiActionNewVersion', {
                method: 'POST',
                body: JSON.stringify({
                    ref_id: quote.assign_id,
                    quote_id: quote.quoteid,
        ...
        <div className={`  pl-0`}>
            <div className="py-2 px-2 flex items-center justify-between bg-blue-100">
                <h3 className="flex items-center">
                    ...
                    {canApprove && quote.quote_status == 0 && quote.submittedtoadmin == "true" && (
                        <button
                    ...
                    {loopUserObject?.can_approve_quote == 1 && quote.ops_approved == 0 && quote.submittedtoadmin == "true" && (
                        <button
            ...
            {(loopUserObject?.id == 1 || loopUserObject?.scopeadmin == 1) && loopUserObject?.fld_admin_type == "SUPERADMIN" && (
                <AlreadyQuoteGiven email_id={queryInfo?.email_id} website_id={queryInfo?.website_id} setAlreadyGiven={setAlreadyGiven} />
            )}
                                ...
                                <div className="mb-2">
                                    ...
                                    <div className="mt-1">
                                        {selectedPlans.map((p, i) => (
                                            <span key={i} className="bg-blue-500 text-white rounded px-2 py-1 f-11 mr-1">{p}</span>
                                        ))}
                                    </div>
                                    {recommendedPlan && (
                                        <p className="mt-2">
                                            <strong>Recommended Plan:</strong> <span className="text-blue-600 bg white border-1 border-blue-600 rounded px-2 py-1 f-12 mr-1">{recommendedPlan}</span>
                                        </p>
                                    )}
                                </div>
                                <div className="grid grid-cols-2 gap-2 px-3 w-full">
                                    {selectedPlans.includes("Basic") && renderPlanCard("Basic", basicInfo, recommendedPlan === "Basic")}
                                    {selectedPlans.includes("Standard") && renderPlanCard("Standard", standardInfo, recommendedPlan === "Standard")}
                                    {selectedPlans.includes("Advanced") && renderPlanCard("Advanced", advancedInfo, recommendedPlan === "Advanced")}
                                </div>
                            </div>
                        )}
                        {quote.client_academic_level && quote.results_section && (
                            <div className="flex gap-4 mb-3">
                                <div className="flex items-center px-1 py-0.5 bg-blue-100 border-l-4 border-blue-500 text-blue-900 shadow-md rounded-lg f-11">
                                    ...
                                    <div className='px-2'>
                                        <h3 className="text-md font-semibold f-12">Academic Level</h3>
                                ...
                                <div className="flex items-center px-1 py-0.5 bg-green-100 border-l-4 border-green-500 text-green-900 shadow-md rounded-lg f-11">
                                    ...
                                    <div className='px-2'>
                                        <h3 className="text-md font-semibold f-12">Results Section</h3>
                                    ...
                                    <div className="flex items-center">
                                        <>
                                            {quote.feasability_status ==
                                                "Completed" &&
                                                quote.submittedtoadmin ==
                                                "false" && quote.user_id == thisUserId && (
                                                    <button
                                                        disabled={
                                                            tlType &&
                                                            tlType == 2
